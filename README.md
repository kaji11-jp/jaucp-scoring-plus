# JAUCP Scoring Tool Plus

日本語版アンサイクロペディアの記事採点ツール（拡張版）。
本プロジェクトは [kongyo2/jaucp-scoring](https://github.com/kongyo2/jaucp-scoring) のフォークであり、独自機能を追加したバージョンです。

AI（LLM）を使用して記事の品質を自動評価します。
...
本フォーク版では、以下の機能が追加されています。

### 1. 🛠️ プロンプト・カスタマイズ機能
AIの採点基準（システムプロンプト）を自由に変更できます。
「設定」画面から、以下のプリセットを選択したり、自分好みに編集したりできます。

- **通常AI (削除主義者)**: デフォルトの辛口レビュアー。
- **厳格なウィキペディアン**: ユーモアを徹底的に排除するモード。アンサイクロペディアの記事をウィキペディアの基準で裁き、面白い記事ほど低評価を下します（逆説的な楽しみ方が可能）。
- **カスタム**: 自分でプロンプトを記述して、独自の採点官を作成できます。

### 2. 🕰️ 採点履歴タイムライン
過去の採点結果を自動的に保存し、「履歴」タブからいつでも確認できます。
履歴をクリックすると、その時点のスコアとアドバイスが復元されるので、修正前後の比較に便利です。

### 3. 📄 Wikitextレポート出力
採点結果画面に「Wiki形式コピー」ボタンを追加しました。
クリックするだけで、アンサイクロペディアのノートページに貼り付けられる形式（表組み＋署名付き）で結果をコピーできます。

### 4. 🌐 ブラウザ版完全対応
Tauri環境だけでなく、通常のWebブラウザでも設定や履歴の保存が動作するように改修しました。

### 5. 🖼️ OGP画像生成
記事をTwitter/Xで共有する際のアイキャッチ画像を簡単に生成できます。ja-ucp-ogpを統合し、GUI上でリアルタイムプレビューしながら生成、そのままPNG でダウンロードできます。
...
- 🖼️ **OGP画像生成**: Twitter/Xシェア用の1200×630px画像をワンクリック生成
...
1. [[Releases](https://github.com/kaji11-jp/jaucp-scoring-plus/releases)](https://github.com/kaji11-jp/jaucp-scoring-plus/releases) ページにアクセス
...
### 4. プロンプトのカスタマイズ（上級者向け）

採点に使用する「システムプロンプト（AIへの指示書）」を自由に編集できます。

1. 設定画面を開く
2. 一番下の「システムプロンプト」欄を編集
3. 「保存」をクリック

デフォルトの「削除主義者気質の辛口レビュアー」に加え、ユーモアを徹底的に排除する「厳格なウィキペディアン（ユーモア減点）」モードも搭載。アンサイクロペディアの記事をウィキペディアの基準で裁くことができます。
「リセット」ボタンでいつでもプリセットの初期状態に戻せます。

### 5. 採点履歴の確認

「履歴」タブをクリックすると、過去の採点結果一覧が表示されます。
リストをクリックすると、その時点の採点結果が復元されます。

### 6. Wikitext形式での出力

採点結果画面の「Wiki形式コピー」ボタンをクリックすると、アンサイクロペディアのノートページに貼り付け可能な形式で結果がクリップボードにコピーされます。

### 7. OGP画像生成

ツイッター/Xで記事をシェアする際のアイキャッチ画像（OGP画像）を生成できます。

1. 「OGP画像」タブをクリック
2. **記事タイトル** と **説明文** を入力（必須）
3. **画像ファイル**を選択（任意）— 274×274pxで右側に配置されます
4. 入力内容に応じてリアルタイムプレビューが更新
5. 「PNG ダウンロード」をクリックして1200×630px の PNG ファイルをダウンロード

**デザイン特性:**
- ゴールドアクセント付きダークテーマ（ja-ucp-ogp完全準拠）
- グラスモーフィズムパネル＆グラデーション背景
- 画像なしでもテキストレイアウトが最適化される
- `ansaikuropedia.org/wiki/` URLピル（リンク表示）付き

### 8. Wikipedia存在確認（ユーティリティ機能）
...
│   │   ├── export.ts     # Wikitext形式エクスポート
│   │   ├── gemini.ts     # Gemini API連携
│   │   ├── history.ts    # 採点履歴管理
│   │   ├── models.ts     # OpenRouterモデル管理
│   │   ├── ogp.ts        # OGP画像生成（HTML5 Canvas）
...
**帰属表記（ja-ucp-ogpの使用）:**
OGP画像生成機能（`src/lib/ogp.ts`）は [ja-ucp-ogp](https://github.com/kongyo2/ja-ucp-ogp) （同じくkongyo2氏作成）を基に、HTML5 Canvas APIを使用したブラウザ互換性のあるバージョンに移植しています。
